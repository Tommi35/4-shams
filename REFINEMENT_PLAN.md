...
- [ ] E1. Update `TODO.md` (new Phase 10 rows) + `PROJECT_STATE.md` (phase, decisions, progress log entry).
- [ ] E2. Sync the temp clone at `C:\Users\xale\AppData\Local\Temp\opencode\deploy-4shams` (rm removed audio, rename, copy changed files), `git add -A` → commit → push.
- [ ] E3. Verify live URL HTTP 200 + one audio file fetch; confirm removed tracks 404 on the live copy.
